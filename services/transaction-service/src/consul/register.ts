...
const logger = new Logger('Consul');
...
export async function registerWithConsul(opts: {
  ...
  if (!consulHost) {
    logger.debug('CONSUL_HOST not set — skipping service registration');
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn(`Consul register failed: ${res.status}`);
      return;
    }
    logger.log(`Registered ${opts.name} with Consul as ${id}`);
    ...
    logger.warn(`Consul unavailable: ${(err as Error).message}`);
  }
}
