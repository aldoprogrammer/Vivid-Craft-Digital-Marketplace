...
function routeLabel(path: string): string {
  ...
  if (clean.includes('/notifications/stream')) return '/notifications/stream';
  if (/^\/orders\/[^/]+$/.test(clean)) return '/orders/:id';
  if (/^\/purchases\/[^/]+$/.test(clean)) return '/purchases/:id';
  if (/^\/reviews\/[^/]+$/.test(clean)) return '/reviews/:id';
  if (/^\/profile\/[^/]+$/.test(clean)) return '/profile/:id';
  return clean;
}
